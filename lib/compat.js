...
var pfx_mac_key = function (pw, salt, iters) {
    if (!Buffer.isBuffer(salt) || salt.length === 0 ||
            salt.length > MAX_PBES2_SALT_LENGTH ||
            !Number.isSafeInteger(iters) || iters < 1 ||
            iters > MAX_KDF_ITERATIONS) {
    ...
    var key = dstu7564.computeHash(
        32,
        Buffer.concat([diversifier, repeated_salt, repeated_password])
    );
    for (var round = 1; round < iters; round++) {
        key = dstu7564.computeHash(32, key);
    }
    return key;
};
...
var pfx_mac = function (pw, mac) {
    ...
    var key = pfx_mac_key(pw, mac.salt, mac.iters);
...
var verify_pfx_mac = function (parsed, pw) {
    ...
    if (mac.algorithm !== 'Dstu7564-256') {
        throw new Error('Unsupported PFX MAC algorithm: ' + mac.algorithm);
    }
